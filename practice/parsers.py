...
def get_text(node, selector, default = ""):
    """
        HTML 태그안에서 '텍스트(글자)'만 안전하게 추출하는 함수

        Args:
            node: 찾을 범위가 되는 HTML
            selector: 찾고 싶은 요소의 CSS선택자
...
def get_attr(node, selector, attr, default=""):
    """
        HTML 태그 안에서 '속성값(href, src등)'을 안전하게 추출하는 함수
...
def parse_stocks(html):
    """
        ...
        동작순서
        1. 텍스트 형태의 HTML을 BeautifulSoup을 이용하여 DOM구조로 변환
        2. 목록에 해당하는 컨테이너(tr.stock-row) 단위로 먼저추출
        3. 해당 컨테이너에서 이름,가격,링크등을 안전하게 추출하여 리스트로 담아서 반환
    """
