...
"""
    * loc vs iloc : 행/열 (인덱싱) 및 범위 선택(슬라이싱)
        ...
        - iloc (Integer Location) : 정수(위치) 기반 선택
         ...
         - 행선택 : 0부터 시작하는 위치 (숫자)
         - 열선택 : 0부터 시작하는 위치 (숫자)
         => 위치 기반으로 기존 슬라이싱과 동일, 끝위치 미포함(제외)
...
result = df.loc[ df['close'] > 200_000, ['code', 'close', 'volume']].heat(3)
print(result)
...
print(indexed.loc['G0001', ['close', 'chageRate']])